...
y_val_pred = np.maximum(model.predict(X_val_scaled), 0)
...
results.to_csv('../results/linear_regression_predictions.csv', index=False)

pd.DataFrame([{
    'Model': 'Linear Regression', 'RMSE': round(rmse, 2), 'MAE': round(mae, 2),
    'R2': round(r2, 4), 'R2_pct': round(r2 * 100, 2),
    'MAPE': round(mape, 2), 'WMAPE': round(wmape, 2)
}]).to_csv('../results/linear_regression_metrics.csv', index=False)

with open('../models/linear_regression_model.pkl', 'wb') as f:
    pickle.dump(model, f)
with open('../models/linear_regression_scaler.pkl', 'wb') as f:
